...
def generate_acos_lut(output_file=None, entries=512, fraction_bits=32):
    ...
    lines.append("    // Binary search to find the appropriate interval")
    lines.append("    int left = 0;")
    lines.append("    int right = kAcosXValues.size() - 1;")
    lines.append("    int idx = 0;")
    lines.append("")
    
    lines.append("    // Handle edge cases")
    lines.append("    if (x >= kAcosXValues[0]) {")
    lines.append("        // Value is >= 1.0, return 0 (or close to it)")
    lines.append("        idx = 0;")
    lines.append("    } else if (x <= kAcosXValues[kAcosXValues.size() - 1]) {")
    lines.append("        // Value is <= 0.0, return π/2 (or close to it)")
    lines.append("        idx = kAcosXValues.size() - 2;  // Use second-to-last point for interpolation")
    lines.append("    } else {")
    lines.append("        // Binary search for the right interval")
    ...
    lines.append("            } else {")
    lines.append("                // Exact match found")
    lines.append("                idx = mid;")
    lines.append("                break;")
    lines.append("            }")
    lines.append("        }")
    lines.append("        ")
    lines.append("        // If we didn't find an exact match, left is now the index")
    lines.append("        // where kAcosXValues[left-1] > x >= kAcosXValues[left]")
    lines.append("        if (left > right) {")
    lines.append("            idx = left - 1;")
    lines.append("        }")
    lines.append("    }")
    lines.append("")
