...
def generate_acos_lut(output_file="acos_lut.h"):
    ...
    # Region 3: 0.93-0.99 denser uniform distribution (512 points)
    num_points3 = 512
    ...
    with open(output_file, "w") as f:
        ...
        f.write(f"constexpr int64_t kPi = {PI}LL;  // π in Q{64-P}.{P} format (π × 2^{P})\n\n")
        ...
        f.write("    constexpr int kRegion1Size = 513;  // 512 + 1\n")
        f.write("    constexpr int kRegion2Size = 387;  // 129 * 3\n")
        f.write("    constexpr int kRegion3Size = 513;  // 512 + 1\n")
        f.write("    constexpr int kRegion4Size = 513;  // 512 + 1\n")
        f.write("    constexpr int kRegion5Size = 257;  // 256 + 1\n\n")
        ...
        f.write("        constexpr int kShift = 9;  // log2(512)\n")
        f.write("        int index = (scaled_x << kShift) / kThreshold_0_8;  // x * 512 / (0.8 * kOne)\n")
        f.write("        constexpr int kMaxIndex = 511;\n")
        f.write("        index = std::min(index, kMaxIndex);\n\n")
        
        f.write("        // Calculate interpolation\n")
        f.write("        int64_t x0 = (index * kThreshold_0_8) >> kShift;  // index * 0.8 * kOne / 512\n")
        f.write("        int64_t dx = scaled_x - x0;\n")
        f.write("        constexpr int64_t kDelta = kThreshold_0_8 >> kShift;  // 0.8 * kOne / 512\n")
        ...
        f.write("        constexpr int kMaxSeg = kSegments - 1;\n")
        f.write("        seg = std::min(seg, kMaxSeg);\n\n")
        ...
        f.write("        constexpr int kPoints = 512;\n")
        f.write("        int index = (rel_x * kPoints) / kScale;\n")
        f.write("        constexpr int kMaxIndex = kPoints - 1;\n")
        f.write("        index = std::min(index, kMaxIndex);\n\n")
        
        f.write("        int idx = base_idx + index;\n")
        f.write("        int64_t x1 = kThreshold_0_93 + (kScale * index) / kPoints;\n")
        f.write("        int64_t x2 = kThreshold_0_93 + (kScale * (index + 1)) / kPoints;\n\n")
        ...
        f.write("        int idx = base_idx + index;\n")
        f.write("        int64_t x1 = kThreshold_0_99 + (kScale * index) / kPoints;\n")
        f.write("        int64_t x2 = kThreshold_0_99 + (kScale * (index + 1)) / kPoints;\n\n")
        ...
        f.write("        constexpr int kPoints = 256;\n")
        f.write("        int index = (rel_x * kPoints) / kScale;\n")
        f.write("        constexpr int kMaxIndex = kPoints - 1;\n")
        f.write("        index = std::min(index, kMaxIndex);\n\n")
        
        f.write("        int idx = base_idx + index;\n")
        f.write("        int64_t x1 = kThreshold_0_999 + (kScale * index) / kPoints;\n")
        f.write("        int64_t x2 = kThreshold_0_999 + (kScale * (index + 1)) / kPoints;\n\n")
