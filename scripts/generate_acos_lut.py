...
def generate_acos_lut(output_file="acos_lut.h"):
    ...
    with open(output_file, "w") as f:
        ...
        f.write("        int index = (scaled_x << 9) / kThreshold_0_8;  // x * 512 / (0.8 * kOne)\n")
        f.write("        index = std::min(index, 511);\n\n")
        
        f.write("        // Calculate interpolation\n")
        f.write("        int64_t x0 = (index * kThreshold_0_8) >> 9;  // index * 0.8 * kOne / 512\n")
        f.write("        int64_t dx = scaled_x - x0;\n")
        f.write("        constexpr int64_t delta = kThreshold_0_8 >> 9;  // 0.8 * kOne / 512\n")
        ...
        f.write("        int64_t x1 = kThreshold_0_93 + (scale * index) / 512LL;\n")
        f.write("        int64_t x2 = kThreshold_0_93 + (scale * (index + 1)) / 512LL;\n\n")
        ...
        f.write("        int idx = base_idx + index;\n")
        f.write("        int64_t x1 = kThreshold_0_99 + (scale * index) / 512LL;\n")
        f.write("        int64_t x2 = kThreshold_0_99 + (scale * (index + 1)) / 512LL;\n\n")
        ...
        f.write("        int idx = base_idx + index;\n")
        f.write("        int64_t x1 = kThreshold_0_999 + (scale * index) / 256LL;\n")
        f.write("        int64_t x2 = kThreshold_0_999 + (scale * (index + 1)) / 256LL;\n\n")
