...
class Primitives {
 public:
    ...
    [[nodiscard]] static auto constexpr OptimizedDivide(int64_t n,
                                                        int64_t d,
                                                        int fractionBits) noexcept -> int64_t {
        // Handle special cases
        if (d == 0) {
        ...
        // Use double division to get initial approximation
        double initial_approximation = static_cast<double>(n) / static_cast<double>(d);

        // Extract sign bits
        int64_t s_n = n >> 63;
        int64_t s_d = d >> 63;

        // Calculate dividend and divisor absolute values using branch-free code
        uint64_t n_abs = (static_cast<uint64_t>(n ^ s_n)) - s_n;
        uint64_t d_abs = (static_cast<uint64_t>(d ^ s_d)) - s_d;
        ...
        // Convert initial approximation to uint64_t representation
        uint64_t result_abs = static_cast<uint64_t>(std::abs(initial_approximation));

        // Verify and refine the approximation using at most 2 iterations
        // This is where we save significant computation compared to full long division
        for (int i = 0; i < 2; i++) {
            // Calculate remainder
            uint64_t prod_hi, prod_lo;
            MultiplyU64(result_abs, d_abs, prod_hi, prod_lo);

            // If product > n_combined, decrease result
            if (prod_hi > n_hi || (prod_hi == n_hi && prod_lo > n_lo)) {
                result_abs--;
            }
            // If product + d <= n_combined, increase result
            else {
                uint64_t sum_lo = prod_lo + d_abs;
                uint64_t sum_hi = prod_hi + (sum_lo < prod_lo ? 1 : 0);

                if (sum_hi < n_hi || (sum_hi == n_hi && sum_lo <= n_lo)) {
                    result_abs++;
                } else {
                    // Result is correct, no further adjustment needed
                    break;
                }
            }
        }

        // Apply sign: If s_result is negative, invert and add 1
        return (static_cast<int64_t>(result_abs ^ s_result)) - s_result;
    }

    // Helper function for multiplication
    static constexpr void MultiplyU64(uint64_t a, uint64_t b, uint64_t& hi, uint64_t& lo) {
