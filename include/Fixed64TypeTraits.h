...
template <int P>
struct is_fixed64_impl<Fixed64<P>> : std::true_type {};
...
struct CompareType<Fixed64<P>, T> {
    using type = Fixed64<P>;
};

template <typename T, int P>
    requires(std::is_arithmetic_v<T> && !IsFixed64<T>)
struct CompareType<T, Fixed64<P>> {
    using type = Fixed64<P>;
};

// Handle comparisons between Fixed64 types
template <int P1, int P2>
struct CompareType<Fixed64<P1>, Fixed64<P2>> {
    using type = std::conditional_t<(P1 > P2), Fixed64<P1>, Fixed64<P2>>;
};

// Specialization for Fixed64 types with the same precision
template <int P>
struct CompareType<Fixed64<P>, Fixed64<P>> {
    using type = Fixed64<P>;
};
